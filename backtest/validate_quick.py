"""Quick validation: 2 years, fewer configs."""
import sys
...
def run_config(cfg: Config, years: list[str]) -> dict:
    ...
    return {
        "config_name": cfg.name, "asset": cfg.asset, "years": years,
...
def main():
    years = ["2023", "2024"]
    configs = [
        Config("WIN_25R_nomart_ML3_SL300", "WIN", 25, 5.0, 0.20, 100.0, 50.0, 3, 300.0, False),
        Config("WIN_25R_mart_ML3_SL300", "WIN", 25, 5.0, 0.20, 100.0, 50.0, 3, 300.0, True),
        Config("WIN_35R_nomart_ML3_SL300", "WIN", 35, 5.0, 0.20, 150.0, 75.0, 3, 300.0, False),
        Config("WDO_15R_nomart_ML3_SL30", "WDO", 15, 0.5, 10.0, 3.0, 1.0, 3, 30.0, False),
        Config("WDO_15R_mart_ML3_SL30", "WDO", 15, 0.5, 10.0, 3.0, 1.0, 3, 30.0, True),
        Config("WIN_25R_nomart_ML3_SL300_news", "WIN", 25, 5.0, 0.20, 100.0, 50.0, 3, 300.0, False, start_h=10, start_m=15),
    ]
    ...
    for i, r in enumerate(all_results, 1):
        print(f"{i}. {r['config_name']:40s} | PnL=R${r['net_pnl']:10,.2f} "
              f"DD={r['max_drawdown_pct']:5.1f}% R/DD={r['return_dd_ratio']:.2f} PF={r['profit_factor']:.2f}", flush=True)

    out_path = PROJECT_ROOT / "reports" / f"robustness_quick_{'_'.join(years)}.json"
